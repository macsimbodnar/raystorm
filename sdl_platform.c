 ...
 *                             SDL UTIL FUNCTIONS
 *****************************************************************************/
internal void sdl_resize_texture(sdl_offscreen_buffer_t* buffer,
                                 ...
                                 int height)
{
  ...
  buffer->memory = mmap(0, bitmap_memory_size, PROT_READ | PROT_WRITE,
...
internal void sdl_process_keyboard_message(game_button_state_t* newState,
                                           bool32 isDown)
{
...
internal void sdl_display_buffer_in_window(sdl_offscreen_buffer_t* buffer,
                                           SDL_Renderer* renderer,
                                           int window_width,
                                           int windowHeight)
{
...
internal void sdl_process_pending_events(
    sdl_state_t* state,
      ...
      case SDL_WINDOWEVENT: {
        switch (event.window.event) {
          ...
          case SDL_WINDOWEVENT_EXPOSED: {
            ...
            sdl_display_buffer_in_window(&g_back_buffer, renderer,
...
internal f32 sdl_process_game_controller_axis_value(i16 value,
                                                    i16 dead_zone_threshold)
{
  f32 result = 0;

  if (value < dead_zone_threshold) {
    result =
        (f32)((value + dead_zone_threshold) / (32768.0f - dead_zone_threshold));
  } else if (value > dead_zone_threshold) {
    result =
        (f32)((value - dead_zone_threshold) / (32767.0f - dead_zone_threshold));
  }

  return result;
}

internal void sdl_process_controller(SDL_GameController* controller,
                                     ...
                                     game_controller_input_t* new_controller)
{
  ...
  new_controller->average_lx = sdl_process_game_controller_axis_value(
      axis_lx, CONTROLLER_AXIS_LEFT_DEADZONE);
  new_controller->average_ly = sdl_process_game_controller_axis_value(
      axis_ly, CONTROLLER_AXIS_LEFT_DEADZONE);

  new_controller->average_rx = sdl_process_game_controller_axis_value(
      axis_rx, CONTROLLER_AXIS_RIGHT_DEADZONE);
  new_controller->average_ry = sdl_process_game_controller_axis_value(
      axis_ry, CONTROLLER_AXIS_RIGHT_DEADZONE);

  new_controller->average_tl = sdl_process_game_controller_axis_value(
      trigger_l, CONTROLLER_AXIS_TRIGGER_DEADZONE);
  ...
  if ((new_controller->average_ly != 0.0f) ||
      (new_controller->average_lx != 0.0f)) {
...
internal void sdl_fill_sound_buffer(sdl_sound_output_t* g_sound_output,
                                    int bytes_to_write,
...
int main(int argc, char* argv[])
{
  ...
  // Init SDL
  CHECK_ERR(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER |
                     SDL_INIT_HAPTIC | SDL_INIT_AUDIO));

  // Create the window
  SDL_Window* window = SDL_CreateWindow("RayStorm", SDL_WINDOWPOS_UNDEFINED,
                                        SDL_WINDOWPOS_UNDEFINED, SDL_WIDTH,
                                        SDL_HEIGHT, SDL_WINDOW_RESIZABLE);
  CHECK_NULL(window);
  ...
  g_sound_output.secondary_buffer_size =
      g_sound_output.samples_per_second * g_sound_output.bytes_per_sample;
  g_sound_output.safety_bytes =
      (int)(((f32)g_sound_output.samples_per_second *
             (f32)g_sound_output.bytes_per_sample / game_update_Hz) /
            2.0f);
  sdl_init_audio(g_sound_output.samples_per_second,
                 g_sound_output.secondary_buffer_size);
  ...
  g_audio_samples =
      (i16*)calloc(g_sound_output.samples_per_second + max_possible_overrun,
  ...
  sdl_state.game_memory_block =
      mmap(base_address, (size_t)sdl_state.total_size, PROT_READ | PROT_WRITE,
  ...
  while (g_running) {
    ...
    game_controller_input_t* old_keyboard_controller =
        get_controller(old_input, 0);
    game_controller_input_t* new_keyboard_controller =
    ...
    *new_keyboard_controller =
    ...
    for (u32 controller_index = 0; controller_index < max_controller_count;
         controller_index++) {
      u32 our_controller_index = controller_index + 1;

      game_controller_input_t* old_controller =
          get_controller(old_input, our_controller_index);
      game_controller_input_t* new_controller =
          get_controller(new_input, our_controller_index);
    ...
    f32 from_begin_to_audio_seconds =
        sdl_get_seconds_elapsed(flip_wall_clock, audio_wall_clock);
    ...
    u32 expected_bytes_until_flip =
        (u32)((seconds_left_until_flip / target_seconds_per_frame) *
    ...
    sound_buffer.sample_count =
        bytes_to_write / g_sound_output.bytes_per_sample;

    bytes_to_write =
        sound_buffer.sample_count * g_sound_output.bytes_per_sample;
...
#if RS_DEBUG
    DEBUG_sdl_time_marker_t* marker =
    ...
    sdl_window_dimension_t dimension = sdl_get_window_dimension(window);
    sdl_display_buffer_in_window(&g_back_buffer, renderer, dimension.width,
                                 dimension.height);
